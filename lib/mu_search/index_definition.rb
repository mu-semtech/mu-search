module MuSearch
  ...
  class IndexDefinition
    ...
    def self.create_composite_sub_definitions(composite_definition, definitions)
      ...
      definitions.map do |name , index_definition|
        name = composite_definition["type"]
        ...
        composite_definition["properties"].each do |composite_prop|
          ...
          if mapped_name
            properties[property_name] = index_definition.properties[mapped_name]
          else
            properties[property_name] = index_definition.properties[property_name]
          end
        end

        CompositeSubIndexDefinition.new(
          name: composite_definition.name,
    ...
    def self.from_config(all_definitions)
      all_definitions.collect do |definition|
        name = definition["type"]
        composite_types = []
        if type_def["composite_types"]
          composite_types = create_composite_subindexes(definition, all_definitions)
        end
    ...
    def related_rdf_types
      if is_regular_index?
        ...
        self.composite_types.map(&:related_rdf_types).flatten
      end
    end
